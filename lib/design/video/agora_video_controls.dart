...
// Forked from CupertinoControls: https://github.com/fluttercommunity/chewie/blob/master/lib/src/cupertino/cupertino_controls.dart
class AgoraVideoControls extends StatefulWidget {
  const AgoraVideoControls({
  ...
  State<StatefulWidget> createState() {
    return _AgoraVideoControlsState();
  }
}

class _AgoraVideoControlsState extends State<AgoraVideoControls> with SingleTickerProviderStateMixin {
  ...
  Widget _buildSubtitleToggle(Color iconColor, double barHeight) {
    // if don't have subtitle, hide button
  ...
  Widget _buildProgressBar() {
    return Expanded(
      child: Padding(
        padding: const EdgeInsets.only(right: 12.0),
        child: Semantics(
          label: "${_toSemanticDuration(_latestValue.duration - _latestValue.position)} restant",
          child: AgoraVideoProgressBar(
            controller,
            onDragStart: () {
              setState(() {
                _dragging = true;
            ...
            colors: AgoraVideoProgressColors(),
          ),
        ),
      ),
    );
  ...
  String _toSemanticDuration(Duration duration) {
    ...
    return '${hours <= 0 ? '' : '$hours heures'} $minutes minutes $seconds secondes';
  }

  void _playPause() {
    SemanticsHelper.announcePlayPause(controller.value.isPlaying);
  ...
  Future<void> _skipBack() async {
    ...
    await controller.seekTo(Duration(milliseconds: max(skip, beginning)));
  ...
  Future<void> _skipForward() async {
    ...
    final skip = (_latestValue.position + const Duration(seconds: 15)).inMilliseconds;
    await controller.seekTo(Duration(milliseconds: min(skip, end)));
    // Restoring the video speed to selected speed
            ...
            (e) => CupertinoActionSheetAction(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    e.toString(),
                    semanticsLabel: e == _selected ? "$e, vitesse courante" : e.toString(),
                  ),
                ],
              ),
            ),
